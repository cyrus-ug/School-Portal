...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare('
                UPDATE students 
                SET student_id_number = ?, first_name = ?, last_name = ?, email = ?, phone = ?, 
            ...
            $success = true;
            header('Location: view-student.php?id=' . $studentId . '&updated=1');
            exit;
        } catch (PDOException $e) {
            ...
            <div class="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-8 py-4 sticky top-0 z-10">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-slate-600 dark:text-slate-400 text-sm mt-1"><?php echo htmlspecialchars($student['first_name'] . ' ' . $student['last_name']); ?></p>
                    </div>
                    <a href="view-student.php?id=<?php echo $studentId; ?>" class="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 transition font-medium">
                        ← Back to Student
                    </a>
                </div>
            </div>
                ...
                <div class="max-w-3xl">
                    ...
                    <form method="POST" class="bg-white dark:bg-slate-900 rounded-xl p-8 border border-slate-200 dark:border-slate-800 space-y-6">
                        ...
                        <div class="flex gap-4 pt-6 border-t border-slate-200 dark:border-slate-800">
                            <button
                                type="submit"
                                class="px-6 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-semibold transition"
                            >
                                Save Changes
                            </button>
                            <a
                                href="view-student.php?id=<?php echo $studentId; ?>"
